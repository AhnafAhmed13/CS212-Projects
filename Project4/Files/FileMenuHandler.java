...
public class FileMenuHandler implements ActionListener {
   ...
   public void readSource(File chosenFile) throws FileNotFoundException {
      ...
      // clearing text areas and array lists for new file input
      LeftTA.setText(null);
   ...
   public void wordList(String line, int num) {
      ...
      while (myTokens.hasMoreTokens()) {
         ...
         // comparing the first character of the words with each vowels and sending them to 'duplicates' method for adding them to their respective lists
         switch (Character.toLowerCase(word.charAt(0))) {
            case 'a': duplicates(listOfAs,word,num);
            break;
            case 'e': duplicates(listOfEs,word,num);
            break;
            case 'i': duplicates(listOfIs,word,num);
            break;
            case 'o': duplicates(listOfOs,word,num);
            break;
            case 'u': duplicates(listOfUs,word,num);
            break;
         } // switch
      } // while
   } // wordList
   ...
   /**
    * duplicates method to check if a duplicate word is already present in the list
    * @param vowelList
    * @param word
    * @param linenum
    */
   public void duplicates(ArrayList<WordLine> vowelList, String word, int linenum) {
      for (WordLine myWord : vowelList) {
         if (word.equals(myWord.word)) { // check for duplicates
            myWord.lines.add(linenum); // add current line to 'lines' arraylist
            return; // return when duplicate has been found, no new node is created
         } // if
      } // for
      vowelList.add(new WordLine(word, linenum)); // if no duplicate is found, create a new node
   } // duplicates
   ...
   public void displayWords(ArrayList<WordLine> vowelList, String vowel) {
      ...
      for (WordLine myWord : vowelList) {
         RightTA.append("\n" + myWord.word + ": ");
         String s = ""; // preliminary string
         String str = ""; // substring to edit
         if (myWord.lines.size() > 1) { // if 'lines' has more than 1 integer
            for (int i : myWord.lines) {
               s += Integer.toString(i) + ", ";
            } // for
            str = s.substring(0, s.length()-2); // to get rid of the last comma
            RightTA.append(str);
         } // if
         else { // if 'lines' has only 1 integer
            RightTA.append(Integer.toString(myWord.lines.get(0)));
         } // else
      } // for
   } // displayWords
